...
我们将 Project2 中需要实现的内容分为逻辑和界面。在 Project 1 中，同学们基本上已经完成了大部分的逻辑操作，这次只需要对一部分原来没有实现和对未完善的部分进行完善，着重重构代码和使用面向对象设计的部分。图形化界面素材在 `audio` 和 `pic` 文件夹中给出。大家也可以自由发挥，打造属于你自己的魔塔，只要展示清晰、明了即可。
...
史莱姆家族们获得了不同程度得暴击率和闪避率（暴击伤害为原伤害的两倍，闪避为躲避此次伤害）
...
骷髅家族们每次攻击有概率造成特殊攻击，特殊攻击无视玩家20%的防御力（向上取整）并且造成120%攻击力的伤害（向上取整）
...
本次课程项目面试时间为**待定**，地点为机房。如有调整会提前通知。 
